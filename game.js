...
function colorMap(){
  activeTerritories().forEach(s=>{
    const nodes = document.querySelectorAll('#map-container #'+CSS.escape(s.abbr)+', #map-container [data-id="'+s.abbr+'"]');
...
function attachAllMapClicks(){
  ...
  activeTerritories().forEach(s => {
    const nodes = wrap.querySelectorAll('#'+CSS.escape(s.abbr)+', [data-id="'+s.abbr+'"]');
    nodes.forEach(path => {
      path.style.cursor='pointer';
      path.onclick = () => onStateClick(s.abbr);
    });
  });
}
